...
import { addLivraison } from "../../service/livraisonservice"
...
const InsertLivraison = ({ show, handleClose, handleAddLivraison }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await addLivraison(formData)
      handleAddLivraison(response.data)
      handleClose()
      ...
      console.error("Erreur lors de l'ajout de la méthode de livraison:", err)
      setError(err.response?.data?.message || "Une erreur est survenue lors de l'ajout de la méthode de livraison")
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      <DialogHeader>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="h6">Ajouter une méthode de livraison</Typography>
        </Box>
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          ...
          >
            {loading ? "Enregistrement..." : "Ajouter la méthode de livraison"}
          </SubmitButton>
        </DialogActions>
      </form>
    </StyledDialog>
...
export default InsertLivraison
